...
document.getElementById('clickbtn').addEventListener('click', () => {
  ...
  if (energyelementvalue <= 0) {
    ...
    document.getElementById('modaldescription').textContent =
      'Обнуление энергии произойдет в 00:00мск';
...
async function saveToDB(currentvalue, initialvalue, energyvalue) {
  try {
    const response = await fetch(
      'https://api.directual.com/good/api/v5/data/rqsttosavescore/rqstSaveScore?appID=b27175e7-b9eb-48bb-a207-e7b7e3c32835&sessionID=',
      {
        method: 'POST',
        body: JSON.stringify({
          score: currentvalue,
    ...
    if (data.result === 'allIsGood') {
      ...
      const energyelement = document.getElementById(
        'game__energy__currentlevel'
      );
...
const mainmenubtn__btn3 = document
  .getElementById('mainmenubtn__btn3')
  .addEventListener('click', () => {
